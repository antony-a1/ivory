...
object IvoryStorage {
  ...
  def multiFactsetLoader(version: FactsetVersion, path: Path, factsets: List[PrioritizedFactset], after: Option[Date]): IvoryScoobiLoader[(Int, String, Fact)] = version match {
