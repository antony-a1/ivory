...
object Versions {
  ...
  def writeFactsetVersionToHdfs(repo: HdfsRepository, version: FactsetVersion, factsets: List[FactsetName]): Hdfs[Unit] = {
    factsets.traverse(fs => for {
      p <- Hdfs.value(new Path(repo.factsetPath(fs), factsetVersionFile))
      e <- Hdfs.exists(p)
  ...
  def readFactsetVersionFromHdfs(repo: HdfsRepository, factset: String): Hdfs[FactsetVersion] = for {
    raw <- Hdfs.readWith(new Path(repo.factsetPath(factset), factsetVersionFile), is => Streams.read(is))
    v   <- Hdfs.fromDisjunction(parseFactsetVersion(raw, repo.path.toString, factset))
  } yield v

  def readFactsetVersionFromS3(repo: S3Repository, factset: String): S3Action[FactsetVersion] = for {
    raw <- S3.getString(repo.bucket, repo.factsetKey(factset) + "/" + factsetVersionFile)
    v   <- Aws.fromDisjunctionString(parseFactsetVersion(raw, "s3://" + repo.bucket + "/" + repo.key, factset))
  } yield v
